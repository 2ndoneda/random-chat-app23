...
export default function SplashScreen({ onComplete }: SplashScreenProps) {
  ...
  return (
    <div
      style={{
        background: "linear-gradient(to bottom right, #f44b7f, #ffb6b9, #ff6661)",
    ...
    >
      <div className="flex flex-col items-center justify-center animate-fade-in max-w-lg w-full">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 mt-4 sm:mt-6">
          ...
          ></div>
          <div
            style={{ backgroundColor: "#ff6661", animationDelay: "0.2s" }}
            className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full animate-bounce shadow-lg"
          ></div>
        </div>
        ...
        {connectionStatus.showConnectionTest && (
          <div className="mt-4 sm:mt-6 bg-white/90 backdrop-blur-sm rounded-xl p-3 sm:p-4 shadow-xl border border-flamingo-200 w-full max-w-sm mx-4">
            <div className="flex items-center gap-2 sm:gap-3">
              <Database className="w-4 h-4 sm:w-5 sm:h-5 text-flamingo-600 flex-shrink-0" />
              ...
              {connectionStatus.isTestingConnection ? (
                <div className="flex items-center gap-1.5 sm:gap-2 ml-auto">
                  <div className="w-3 h-3 sm:w-4 sm:h-4 border-2 border-flamingo-300 border-t-flamingo-600 rounded-full animate-spin"></div>
                  <span className="text-xs sm:text-sm text-flamingo-700">
                    Testing...
                  </span>
                </div>
